...
export const validatePostData = (postData, propertyType) => {
  ...
  if (
    !postData.price ||
    isNaN(parseInt(postData.price)) ||
    parseInt(postData.price) <= 0
  ) {
    errors.push('Price is required and must be a positive number');
  }
  if (
    !postData.address ||
    typeof postData.address !== 'string' ||
  ...
  if (propertyType === 'land') {
    if (postData.bedroom !== undefined)
      errors.push('Bedroom count should not be specified for land properties');
    if (postData.bathroom !== undefined)
      errors.push('Bathroom count should not be specified for land properties');
  } else {
    if (
      !postData.bedroom ||
      isNaN(parseInt(postData.bedroom)) ||
      parseInt(postData.bedroom) < 1
    ) {
      errors.push('Bedroom count is required and must be at least 1');
    }
    if (
      !postData.bathroom ||
      isNaN(parseInt(postData.bathroom)) ||
      parseInt(postData.bathroom) < 1
    ) {
...
export const validatePostDetail = (postDetail, propertyType) => {
  ...
  ['school', 'bus', 'restaurant'].forEach((field) => {
    if (
      postDetail[field] === undefined ||
      isNaN(parseInt(postDetail[field])) ||
      parseInt(postDetail[field]) < 0
    ) {
      errors.push(
        `${field.charAt(0).toUpperCase() + field.slice(1)} distance must be a non-negative number`
      );
    }
  });
  ...
  if (
    !postDetail.size ||
    isNaN(parseInt(postDetail.size)) ||
    parseInt(postDetail.size) <= 0
  ) {
  ...
  if (propertyType === 'land' && postDetail.pet !== undefined) {
    ...
    !propertyType === 'land' &&
    (!postDetail.pet || !['allowed', 'not-allowed'].includes(postDetail.pet))
  ) {
